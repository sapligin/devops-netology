...
Удалить из ARP таблицы только один нужный IP в Linux можно командой `sudo ip neigh del 192.168.100.25 dev eth0`, где `192.168.100.25` - это IP адрес для которого мы удалем кеш из ARP.  
Удалить из ARP таблицы только один нужный IP в Windows можно командой `arp -d 192.168.100.25`, где `192.168.100.25` - это IP адрес для которого мы удалем кеш из ARP.